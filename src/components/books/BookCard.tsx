...
import { truncateAddress, formatUnixDate } from "../../utils/bookCovers";
...
export const BookCard: React.FC<BookCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative h-52 p-6 flex flex-col justify-between overflow-hidden select-none border-b border-[#E5E5E5]"
        style={{
      ...
      >
        {/* Subtle Spine Line */}
        <div
          className="absolute top-0 bottom-0 left-3 w-[2px] opacity-40"
          style={{ backgroundColor: theme?.spine || "#333A42" }}
        />
        {/* Subtle Paper Grain / Edge */}
        <div
          className="absolute top-0 bottom-0 left-0 w-3 opacity-20"
          style={{
            background:
        ...
        {/* Top Header: Spine Mark & Book ID */}
        <div className="flex items-center justify-between text-[11px] font-mono-ledger opacity-75 pl-3">
          <span>FUL · LEDGER #{book.id}</span>
          <BookOpen className="w-3.5 h-3.5" />
        </div>

        {/* Middle: Serif Title and Author */}
        <div className="pl-3 my-auto py-2">
          <h3 className="font-serif-academic text-base sm:text-lg font-medium leading-snug line-clamp-3">
            {book.title}
          </h3>
          <p
            className="text-xs mt-1.5 line-clamp-1 font-sans opacity-85"
            style={{ color: theme?.subtext || "#94A3B8" }}
          >
            {book.author}
          </p>
        </div>
        ...
        {/* Bottom Cover Accent */}
        <div className="pl-3 flex items-center justify-between text-[10px] font-mono-ledger opacity-60 uppercase tracking-wider">
